...
class Other extends React.Component {
  constructor () {
	...
	async componentDidMount(){
		...
		let blacklist=[];
		axios.get("http://"+host+'/getshow').then(res => {
			let getshow=res.data
			console.log(getshow)
			let data={Data:[],Website:[]}
			if (getshow&&getshow.length>0){
				getshow.map((show)=>{
					if (show.Cmd=="share"){
						data.Data.push(show.Data)
					}else if(show.Cmd=="shareWeb"){
						data.Website.push(show.Data)
					}
				})
			}
			console.log(data)
